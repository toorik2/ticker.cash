...
// Covenant enforces a 30 s minimum stride (Oracle.cash:73-74). Used as the
// pre-measurement fallback before the first cycle is observed.
const DEFAULT_CYCLE_STRIDE_SEC = 30;
...
async function buildSnapshot(): Promise<Stats> {
  ...
  // ─── compose ────────────────────────────────────────────────────────
  // Cycle stride: prefer the measured average; fall back to the covenant
  // minimum (30 s). The covenant enforces a floor, not the typical value.
  ...
  if (oracleState) {
    recordHistory({
      ...
      medianUsd: oracleState.medianUsd,
      slotsCurrent,
      cycleStrideSec,
    });
  }
